...
#ifdef USART_DEBUG
const int THRESHOLD = 2040;  // Adjust this number to avoid noise when idle
#else
...
void setup() {
  ...
  BLEDevice::init("ESP32 Sensor Kit");
...
void loop() {
  ...
  if (intFlag == 1) {
    if (adcData.HR[3] != '0') {
      appendFile(SD, "/PulseSensor_BLE.txt", &(rtc.getTime("%B %d %Y %H:%M:%S"))[0]);
      appendFile(SD, "/PulseSensor_BLE.txt", "\t\t");
      appendFile(SD, "/PulseSensor_BLE.txt", adcData.HR);
      appendFile(SD, "/PulseSensor_BLE.txt", "\r\n");
    }
  }
  adcData.HR[3] == '0';
 ...
 * BLE
 */
void bleTask(void *pvParameters) {  //蓝牙外发数据
  while (1) {
    if (deviceConnected) {
      pCharacteristic->setValue("SPO2:0");
      pCharacteristic->notify();  // Send the value to the app
      pCharacteristic->setValue(adcData.HR);
      pCharacteristic->notify();  // Send the value to the app
    }
    delay(600);
  }
