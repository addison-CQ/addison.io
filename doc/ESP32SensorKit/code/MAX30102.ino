...
void loop() {
  ...
  if (intFlag == 1) {
    if (uartData.HR[0] == 'H' && uartData.HR[1] == 'R') {
      appendFile(SD, "/MAX30102.txt", &(rtc.getTime("%B %d %Y %H:%M:%S"))[0]);
      appendFile(SD, "/MAX30102.txt", "\t\t");
      appendFile(SD, "/MAX30102.txt", uartData.HR);
      appendFile(SD, "/MAX30102.txt", "\r\n");
    }
    if (uartData.SPO2[0] == 'S' && uartData.SPO2[1] == 'P' && uartData.SPO2[2] == 'O' && uartData.SPO2[3] == '2') {
      appendFile(SD, "/MAX30102.txt", &(rtc.getTime("%B %d %Y %H:%M:%S"))[0]);
      appendFile(SD, "/MAX30102.txt", "\t\t");
      appendFile(SD, "/MAX30102.txt", uartData.SPO2);
      appendFile(SD, "/MAX30102.txt", "\r\n");
    }
  }
